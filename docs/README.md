...
Two related fields are deliberately **not** covered, because neither is expanded either: `languageServers[*].cmd[0]` keeps `PATH` lookup semantics (so a bare `rust-analyzer` still works), and `languageServers[*].settings` / `initializationOptions` / `clientCapabilities` are passed through untouched (the first two to the downstream server, the last into the advertised capabilities).
...
| `clientCapabilities` | Override the client capabilities kakehashi advertises to this server, deep-merged last over what kakehashi would send (so values written here always win). Keys can only be set, not removed: disable a boolean capability with an explicit `false` — e.g. `clientCapabilities = { window = { workDoneProgress = false } }` stops a progress-chatty server (basedpyright-style `$/progress` floods) at the source — and disable an object-typed capability via its inner boolean (`window = { showDocument = { support = false } }`). `general.positionEncodings` is protected (reverted with a warning): kakehashi's coordinate translation requires UTF-16. Consumed at `initialize` only; changing it relaunches the server's connections. |
