...
| `forwardShowMessage` | Forward the server's `window/showMessage` notifications to the editor, prefixed with `[kakehashi:<server>]`. Default `false` so noisy servers cannot spam popups. (`window/logMessage` is always forwarded with the same prefix.) |
