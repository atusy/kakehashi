...
- Moniker / Inlay Hint (incl. `inlayHint/resolve` routed back to the exact producing host or injection server; lazy text edits and label locations are translated, while content-stale or non-contiguous injection hints fail soft) / Inline Value (request and result ranges are translated for the selected injection)
...
`textDocument/semanticTokens/full/delta` remains native-only. By default the host layer is
tried after
...
- **Current effect**: the `virt` layer answers inside injection regions, and
  ...
  native span. Only `textDocument/semanticTokens/full/delta` stays native-only.
