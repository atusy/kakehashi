...
| `minPublishIntervalMs` | For `textDocument/publishDiagnostics`, the minimum interval between editor-facing publishes during rapid changes. Default: `1000`; `0` publishes every changed merge; values above 24 hours are capped at 24 hours. This does not affect pull diagnostics or refresh requests. |
