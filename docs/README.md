...
- Moniker / Inlay Hint (incl. `inlayHint/resolve` routed back to the exact producing host or injection server; lazy text edits and label locations are translated, while content-stale or non-contiguous injection hints fail soft)
- Code Lens (incl. `codeLens/resolve` routed back to the origin server for both injection- and host-layer lenses; host-layer payloads and coordinates pass through verbatim, while injection resolution fails soft when the region was moved or invalidated since the lens was produced)
...
- `workspace/executeCommand` (commands surfaced through code actions and inlay-hint label parts route to their producing connection; available even without `codeActionLiteralSupport`). When the client supports dynamic registration, both raw and connection-encoded names from each downstream's initialize result are registered. Clients that require registered ids need this dynamic registration and a downstream that advertises the command; downstream commands omitted from that initial list, including later dynamic registrations, are not registered upstream. The raw-name palette registry is session-global and carries no workspace context: when several live connections advertise the same raw name, kakehashi refuses the invocation and reports a `window/logMessage` warning. Use the connection-encoded entry to select a producer, or narrow `workspaceMarkers`, enable `preferSharedInstance`, or disable one of the servers.
...
| `maxFanOut` | Maximum number of servers to query. `null` or omitted = no configured limit (default). `0` = disable fan-out entirely. Positive integer = cap at N servers. Priority servers are selected first when limiting. Negative values are treated as no configured limit. The sequential `concatenated` formatting pipeline additionally has an internal 64-step safety ceiling. |
