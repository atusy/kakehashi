...
| `clientCapabilities` | Override the client capabilities kakehashi advertises to this server, deep-merged last over what kakehashi would send (values written here win, apart from two protected fields). Keys can only be set, not removed: disable a boolean capability with an explicit `false` — e.g. `clientCapabilities = { window = { workDoneProgress = false } }` stops a progress-chatty server (basedpyright-style `$/progress` floods) at the source — and disable an object-typed capability via its inner boolean (`window = { showDocument = { support = false } }`). Protected (reverted with a warning): `general.positionEncodings` — kakehashi's coordinate translation requires UTF-16 — and `workspace.workspaceEdit.changeAnnotationSupport` — annotated edits would lose their confirmation gate in the bridge. Consumed at `initialize` only; changing it relaunches the server's connections. |
