...
**Bridge routing divergence (caret-shaped methods).** This section governs *node containment* — the question "which node is the cursor on?". Bridge *routing* asks a different question — "where is the user typing?" — and for the methods whose request position is the insert-mode caret (completion, signatureHelp, linkedEditingRange, onTypeFormatting — the authoritative list is `region_boundary_for_method`), the bridge's region lookup adds a fallback: when no region contains the byte half-open, a region whose end byte equals the cursor **and** whose end sits mid-line (non-zero end column) or at the document's end is accepted (`RegionBoundary::CaretEndFallback` in `src/language/injection/discovery.rs`). The caret after the last typed character of a mid-line-ending injection (a vim `!cmd`, an embedded string) is that region's end byte, and it maps to the virtual document's end-of-content — a valid LSP position. A region ending at column 0 before the document's end is deliberately excluded, so "cursor at the closing fence" stays outside; the end-of-document case (an unclosed block whose content ends on the just-typed newline) mirrors the End-of-Document Exception below. Half-open containment always wins first, so an adjacency `end(A) == start(B)` still resolves to B. Node resolution (`kakehashi/node`) is unaffected.

**Which span the bridge measures against.** Both the half-open path and that fallback judge membership on a region's **effective** span — the raw `@injection.content` node after its runtime range directives (`#offset!` / `#trim!`; `effective_content_range` in `src/language/injection/discovery.rs`) — which is the same span the resolved region, its virtual document, and every coordinate translation are built from. A caret on bytes a directive trimmed (markdown frontmatter's closing `---`, a string literal's quotes) is therefore outside the region for *every* method, point-shaped ones included, and an adjacent region starting there can win the lookup; bytes a directive *adds* past the raw node are inside it. Child-exclusion gaps (blockquote `` > `` prefixes, excluded named children) are deliberately **not** subtracted from that span at the bridge: bridge translation models a region as one contiguous byte range plus per-line column offsets, so a lookup that rejected gap bytes while translation still mapped them would only trade an over-permissive class for a mistranslating one (tracked as issue #996 item 6, together with the translation model it needs). `kakehashi/node`'s layer walk does subtract them, because it parses with real `included_ranges` and has no such flat mapping.

Two consequences worth stating, because both look like inconsistencies otherwise. **Region ordering is raw-derived**: both scans take the first match in `collect_all_injections`'s document order, which is sorted by raw span, so "first match" means "outermost" only while no directive shifts a span — same-range alternate languages depend on that raw/pattern order for their documented query-order priority, so it stays. **Region *identity* is raw-derived too**: `calculate_region_id` and `resolve_by_region_id` both key on the raw `content_node` bytes, because a region id answers "which host node is this?" rather than "which bytes are injected". The two are symmetric, so region-id stability is unaffected by the span the lookup measures. A region an `#offset!` collapses to zero width is routable at its collapse byte under the caret rule — that position is the whole (empty) injection — while half-open declines it for want of any interior. Zero width grants no exemption from the column-0 rule, though: a collapse onto a line start is the closing-fence shape (an *empty* frontmatter collapses exactly there) and stays outside, while a mid-line collapse routes.
...
- **Only fires at document end of a non-empty document**: the exception requires `e == L` exactly and `L > 0`. Interior boundaries (cursor at the end of an injection but before the document end) keep the unmodified half-open behavior *for node containment* — bridge routing's caret fallback, above, is the sole divergence — so "cursor at closing fence" still falls out of the injection into the host.
