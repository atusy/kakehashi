...
### Configuration is trusted executable input

Loading a project configuration means trusting it with the privileges of the
kakehashi process. The discovered `kakehashi.toml` is loaded automatically;
kakehashi has no workspace-trust prompt, allowlist, or per-key execution gate.
Version control does not make that file trusted. Review the configuration and
its referenced files before enabling kakehashi in an unfamiliar workspace.
The same trust requirement applies when implicit configuration is reloaded
following a workspace-root change.

This authority is broader than selecting presentation settings:

- `languages.<language>.parser` selects a native library loaded into the server
  process. Relative parser paths are anchored to the containing config file.
- `searchPaths` can select native parser libraries through runtime discovery,
  even without an explicit `parser` value.
- `languageServers.<server>.cmd` selects a program and arguments to execute when
  that server starts, including an eager `forceStart` configuration.
- `baseConfigFiles` imports settings with the same authority. The trust review
  therefore includes those files and the code/resources they select.

We retain this trusted-on-load model for configuration compatibility and the
existing ability to share project toolchains. The alternative is to gate
project settings behind client-provided trust or a local allowlist. That would
need a separate configuration contract covering indirect executable inputs
such as parser search paths and base files, not merely filtering `cmd` and
`parser`. No such gate is implemented by this decision.

For a session that must not discover project configuration, launch with
`--config-file /absolute/path/to/trusted.toml`. Explicit entries replace both
implicit user and project entries; keep the file and any `baseConfigFiles`
outside untrusted project control. Client-provided settings remain another
trusted input. This selects configuration sources; it is not a sandbox for
parsers, downstream servers, or the project configuration those servers may
load themselves.

The benefit is compatibility with configured native parsers and project
commands. The cost is that enabling kakehashi with configuration from an
untrusted repository can execute repository-controlled code without further
confirmation. The bridge's document-content restrictions do not mitigate a
malicious configuration author. Its [security model](language-server-bridge.md#security-model)
assumes this configuration trust boundary.

Confirm this contract at the configuration loader's explicit/implicit branch
(`src/lsp/settings.rs`), parser path resolution and loading
(`src/language/query_loader.rs`, `src/language/coordinator.rs`), and downstream
process spawning (`src/lsp/bridge/connection.rs`). Any future trust gate must
update both this decision and the user-facing configuration documentation.
