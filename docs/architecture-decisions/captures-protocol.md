...
- `true` — run it across **every** layer: the host first, then each injection region in document order, recursing up to `MAX_INJECTION_DEPTH` injected layers, the same bound as cursor selection and node resolution. Each layer resolves **its own language's** `queries/<lang>/<kind>.scm`; a layer whose language has no kind file contributes nothing. Result nodes carry a tree-scope token identifying the resolved language, depth, and complete included byte ranges. Overlapping siblings therefore have distinct identities and resolve in their own trees through `kakehashi/node/*`; see [lazy-node-identity-tracking](lazy-node-identity-tracking.md#layer-discriminator-considered-options).
