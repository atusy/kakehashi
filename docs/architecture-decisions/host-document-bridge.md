...
- **Dispatch**: implemented for every bridged request method. Because the
  ...
  semanticTokens/range, semanticTokens/full, and semanticTokens/full/delta.
  ...
  injection-only results cannot erase the rest of the document. Full/delta
  recomputes that same overlay before diffing the editor-visible baseline.
