...
- **Dispatch**: implemented for every bridged request method. Because the
  ...
  inlayHint, documentSymbol, documentLink, documentColor, foldingRange, codeLens,
  ...
  layer strategy. Not covered: semantic tokens (native-only) and experimental
  colorPresentation. `completionItem/resolve` routes by
