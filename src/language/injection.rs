...
impl CacheableInjectionRegion {
    ...
    /// On the first line (virtual line 0), also subtracts the column offset.
    ///
    ...
    pub fn translate_host_to_virtual(
        ...
        let virtual_line = host_pos.line.saturating_sub(self.line_range.start);
        let virtual_character = if virtual_line == 0 {
            host_pos.character.saturating_sub(self.start_column)
        } else {
            host_pos.character
        };
        tower_lsp_server::ls_types::Position {
    ...
    /// On the first line (virtual line 0), also adds the column offset.
    pub fn translate_virtual_to_host(
        &self,
        virtual_pos: tower_lsp_server::ls_types::Position,
    ) -> tower_lsp_server::ls_types::Position {
        let host_character = if virtual_pos.line == 0 {
            virtual_pos.character.saturating_add(self.start_column)
        } else {
...
mod tests {
    ...
    // ============================================================
    // Tests for column-aware position translation
    // ============================================================

    #[test]
    fn test_translate_host_to_virtual_with_column_offset_first_line() {
        use tower_lsp_server::ls_types::Position;

        // Inline injection starting at column 4 (e.g., code inside a markdown paragraph)
        let region = CacheableInjectionRegion {
            language: "lua".to_string(),
            byte_range: 50..80,
            line_range: 5..5,
        ...
        // Host position on the first line of the injection: character should be adjusted
        let host_pos = Position {
            line: 5,
            character: 10,
        };
        ...
        assert_eq!(virtual_pos.line, 0);
        assert_eq!(
            virtual_pos.character, 6,
            "First line character should be adjusted: 10 - 4 = 6"
        );
    }

    #[test]
    fn test_translate_host_to_virtual_with_column_offset_non_first_line() {
        use tower_lsp_server::ls_types::Position;
        ...
        // Host position on a non-first line: character should NOT be adjusted
        let host_pos = Position {
            line: 6,
            character: 10,
        };
        ...
        assert_eq!(virtual_pos.line, 1);
        assert_eq!(
            virtual_pos.character, 10,
            "Non-first line character should remain unchanged"
        );
    }

    #[test]
    fn test_translate_virtual_to_host_with_column_offset_line_zero() {
        use tower_lsp_server::ls_types::Position;
        ...
        // Virtual line 0: character should be adjusted
        let virtual_pos = Position {
            line: 0,
            character: 6,
        };
        ...
        assert_eq!(host_pos.line, 5);
        assert_eq!(
            host_pos.character, 10,
            "Virtual line 0 character should be adjusted: 6 + 4 = 10"
        );
    }

    #[test]
    fn test_translate_virtual_to_host_with_column_offset_non_zero_line() {
        use tower_lsp_server::ls_types::Position;
        ...
        // Virtual line 1+: character should NOT be adjusted
        let virtual_pos = Position {
            line: 1,
            character: 6,
        };
        ...
        assert_eq!(host_pos.line, 6);
        assert_eq!(
            host_pos.character, 6,
            "Non-zero virtual line character should remain unchanged"
        );
    }

    #[test]
    fn test_translate_host_to_virtual_column_offset_saturates_on_underflow() {
        use tower_lsp_server::ls_types::Position;

        // Race condition: character < start_column on first line
        let region = CacheableInjectionRegion {
            language: "lua".to_string(),
            byte_range: 50..80,
            line_range: 5..5,
        ...
        let host_pos = Position {
            line: 5,
            character: 3, // Less than start_column (10)
        };
        let virtual_pos = region.translate_host_to_virtual(host_pos);

        assert_eq!(virtual_pos.line, 0);
        assert_eq!(
            virtual_pos.character, 0,
            "Column underflow should saturate to 0"
        );
    }
