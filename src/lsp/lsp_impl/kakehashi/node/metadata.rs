...
//! `{ "kind": "fenced_code_block" }`), matching the shape `kakehashi/textDocument/node/text`
...
//! native space, the same one `kakehashi/textDocument/node/text` slices against. Line/column
...
//! `kakehashi/textDocument/node/range` endpoint.
...
impl Kakehashi {
    /// `kakehashi/textDocument/node/kind` — the node's grammar symbol name (`Node::kind`).
    pub async fn kakehashi_node_kind(&self, params: NodeIdParams) -> Result<Value> {
        scalar_accessor!(self, params, "kind", |n| n.kind())
    }

    /// `kakehashi/textDocument/node/grammarName` — the node's grammar name, which differs
    ...
    /// `kakehashi/textDocument/node/isNamed` — whether the node is *named* (vs an anonymous
    ...
    /// `kakehashi/textDocument/node/isExtra` — whether the node is an *extra* (e.g. a comment
    ...
    /// `kakehashi/textDocument/node/hasError` — whether the node's subtree contains a syntax
    ...
    /// `kakehashi/textDocument/node/isError` — whether the node itself is an `ERROR` node,
    /// per `Node::is_error`.
    ...
    /// `kakehashi/textDocument/node/isMissing` — whether the node is a zero-width `MISSING`
    ...
    /// `kakehashi/textDocument/node/startByte` — the node's start byte (UTF-8, host coords),
    /// per `Node::start_byte`.
    ...
    /// `kakehashi/textDocument/node/endByte` — the node's end byte (UTF-8, host coords),
    /// per `Node::end_byte`.
    ...
    /// `kakehashi/textDocument/node/byteRange` — the node's `[startByte, endByte)` span
    ...
    /// `kakehashi/textDocument/node/childCount` — number of children (named + anonymous),
    ...
    /// `kakehashi/textDocument/node/namedChildCount` — number of *named* children, per
    ...
    /// `kakehashi/textDocument/node/descendantCount` — number of descendants including the
    ...
    /// `kakehashi/textDocument/node/toSexp` — the node's subtree as an s-expression string,
