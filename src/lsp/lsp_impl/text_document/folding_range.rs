...
impl Kakehashi {
    pub(crate) async fn folding_range_impl(
        ...
        let Ok(uri) = uri_to_url(&lsp_uri) else {
            log::warn!("Invalid URI in foldingRange: {}", lsp_uri.as_str());
            return Ok(None);
        };

        log::debug!("foldingRange called for {}", uri);

        // Get document snapshot (minimizes lock duration)
        let snapshot = match self.documents.get(&uri) {
            None => {
                log::debug!("foldingRange: No document found for {}", uri);
                return Ok(None);
            }
            Some(doc) => match doc.snapshot() {
                None => {
                    log::debug!("foldingRange: Document not fully initialized for {}", uri);
                    return Ok(None);
                }
        ...
        let Some(language_name) = self.document_language(&uri) else {
            log::debug!(target: "kakehashi::folding_range", "No language detected");
            return Ok(None);
        };
        ...
        let mut outer_join_set: JoinSet<Option<Vec<FoldingRange>>> = JoinSet::new();
        ...
        for resolved in all_regions {
            ...
            outer_join_set.spawn(async move {
                ...
                .await;
                match result {
                    FanInResult::Done(ranges) => ranges,
        ...
        let all_ranges = crate::lsp::aggregation::region::collect_region_results_with_cancel(
            outer_join_set,
            cancel_rx,
            |acc, opt: Option<Vec<FoldingRange>>| {
