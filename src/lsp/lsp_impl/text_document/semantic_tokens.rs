...
impl Kakehashi {
    ...
    async fn semantic_delta_has_applicable_bridge(
        ...
        let owned_regions;
        let regions = if let Some(resolved) = snapshot.regions.as_ref()
            && resolved.generation == generation
        {
            resolved.whole_document.as_slice()
        } else {
    ...
    async fn semantic_tokens_range_host_layer(
                        ...
                        std::sync::Arc::new(move || {
                            documents.get(&host_uri).and_then(|document| {
                                (document.incarnation() == incarnation
                                    ...
                                    .then(|| (document.text_arc(), content_version))
                            })
                        });
                    task.pool
        ...
        .await;
        let tokens = self
            .host_layer_result(result, &ctx, METHOD, |won| won)
            .await?;
        if !self.semantic_range_snapshot_is_current(lsp_uri, incarnation, content_version) {
            return Ok(None);
        }
