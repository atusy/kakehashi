...
impl VirtualDocumentUri {
    ...
    /// Same directory as the host so downstream servers can resolve relative
    /// imports and discover project configs (pyproject.toml, tsconfig.json, …).
    ...
    /// like lua-language-server recognise the file type. The url crate
    /// percent-encodes `region_id` (defence-in-depth; ULIDs are alphanumeric).
