...
impl LanguageServerPool {
    ...
    pub(crate) async fn sync_workspace_document_to_handle(
        ...
        sync_host_document(
            ...
            &HostDocument {
                ...
                language_id,
                text,
                // Workspace-diagnostic syncs carry no store revision: the text is
                // synced verbatim and must not move the revision watermark.
                revision: None,
            },
            None,
            handle.key(),
    ...
    /// synchronization lock and must return the exact current revision; a
    /// superseded request is rejected before it can sync stale text.
    #[allow(clippy::too_many_arguments)]
