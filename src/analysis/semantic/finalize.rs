...
/// then `depth` (injection depth), then `node_depth` (deeper tree nodes
/// are more specific — child beats parent), then `pattern_index` (later
/// patterns win within same node depth).
fn token_priority(t: &RawToken) -> (u32, usize, usize, usize) {
    (t.priority, t.depth, t.node_depth, t.pattern_index)
}
...
/// If you add a new path that emits multiline tokens, ensure it also handles prefix widths.
...
/// Priority is determined by `(priority DESC, depth DESC, node_depth DESC, pattern_index DESC)`.
///
