...
There is no cross-invocation discovery state to invalidate: each populate call uses its supplied tree, text and query, and existing cancellation/generation checks govern publication. The full-query oracle exercises changed text, incremental and fresh parses, query replacement, enclosing/combined regions, text predicates and shifted boundaries. In-process lifecycle tests also drive actual didClose/didOpen handlers and the shared settings-reload transaction with a changed query file, using large documents and a two-worker compute pool. They compare complete collector descriptors against the full-query oracle on the published tree/text/query, compare the published owned regions, and independently check the new incarnation/text or generation/language/offset. These tests do not require an external LSP process.
